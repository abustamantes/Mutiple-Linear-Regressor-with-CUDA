...
__global__ void train(float* x, float* y, float* theta, float alpha, int m, int n) // Define kernel function 'train'
{
    ...
    if (tid < n) {                                  // Check if thread ID is within range
        float sum = 0.0;                            // Initialize sum to zero
        for (int i = 0; i < m; i++) {               // Loop through training examples
            sum += (theta[0] + theta[1] * x[i] + theta[2] * x[i + m]) - y[i];  // Compute sum of errors
        }
        if (tid == 0) {                             // Check if thread is the first thread in the block
            theta[0] -= alpha * (1.0 / m) * sum;     // Update theta[0]
        }
        else if (tid == 1) {                        // Check if thread is the second thread in the block
            sum *= x[tid - 1];                      // Multiply sum by x[0]
            theta[tid] -= alpha * (1.0 / m) * sum;   // Update theta[1]
        }
        else if (tid == 2) {                        // Check if thread is the third thread in the block
...
class Regressor {                                  // Define Regressor class
private:
...
public:
    Regressor(int m, int n)                        // Constructor
    {
        ...
        for (int i = 0; i < n; i++) {               // Initialize theta to zero
            theta[i] = 0.0;
        }
...
*/
    ...
    float x_train[] = {
        1.0, 2.0, // First data point: x = [1.0, 2.0]
        2.0, 3.0, // Second data point: x = [2.0, 3.0]
        3.0, 4.0, // Third data point: x = [3.0, 4.0]
        4.0, 5.0  // Fourth data point: x = [4.0, 5.0]
    };

    float y_train[] = {
        4.0, // First data point has output y = 4.0
        6.0, // Second data point has output y = 6.0
        8.0, // Third data point has output y = 8.0
        10.0 // Fourth data point has output y = 10.0
    };

    // example test data
    float x_test[] = {
        5.0, 6.0 // Test data point: x = [5.0, 6.0]
    };
    ...
    // test model
    float y_pred = regressor.predict(d_x_test); // Predict the output for the test data point
